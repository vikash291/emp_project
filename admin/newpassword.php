...
{
    $str=$_REQUEST['mail'];
    $q1=mysql_query("select * from admin where astr='$str'");
    ...
    if($nrow>0){
          ...
          if(isset($_POST['sub']))
          {
              $qs=$_REQUEST['mail'];
              $npw=$_POST['txtemail'];
              $query=mysql_query("update admin set password='$npw',astr='' where id='$upeid'");
...
$npw=$_POST['txtemail'];
$query=mysql_query("update admin set password='$npw' where astr='$qs'");
if(mysql_affected_rows() > 0){
echo "<script>alert('Password Changed Successfully')</script>";
